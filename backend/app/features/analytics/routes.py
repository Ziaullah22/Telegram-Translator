...
async def get_admin_account_ranking(
    ...
    Note: Real implementation should verify admin JWT. 
    Using a simpler check for demo or assuming restricted router.
    """
    query = """
    WITH response_pairs AS (
        SELECT 
    ...
    JOIN telegram_accounts ta ON rp.telegram_account_id = ta.id
    GROUP BY ta.id, ta.display_name

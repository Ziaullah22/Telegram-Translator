...
export default function ActiveSessionsModal({ isOpen, account, onClose }: ActiveSessionsModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/30 animate-fade-in" onClick={onClose}>
            <div id="sessions-modal-container" className="bg-white dark:bg-[#212121] rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden animate-scale-in flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-white/5">
                    <h3 className="text-[19px] font-medium text-gray-900 dark:text-white">Active Sessions</h3>
                    <button id="sessions-modal-close-btn" onClick={onClose} className="text-gray-400 hover:text-gray-600 dark:hover:text-white p-1 rounded-full transition-colors">
                        <X className="w-5 h-5" />
                    </button>
                </div>

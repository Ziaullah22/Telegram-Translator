    ...
    <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-[#0f172a] p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8">
          ...
          {/* Right Column: 24h Activity Log */}
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white dark:bg-[#1e293b] rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden flex flex-col h-[600px] animate-slide-up">
              <div className="px-6 py-5 border-b border-gray-100 dark:border-white/5 flex-shrink-0">
                ...
                <p className="text-[11px] text-gray-400 mt-1">Chronological record of sent and received messages.</p>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar">
                {activity24h.length === 0 ? (
                  <div className="py-20 text-center text-gray-400">
                    ...
                    <p className="text-sm">No message activity recorded in the last 24 hours.</p>
                  </div>
                ) : (
                  activity24h.map((item, idx) => (
                    <div key={idx} className="flex gap-3 text-xs leading-normal">
                      <div className="flex-shrink-0 flex flex-col items-center">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${item.is_outgoing ? 'bg-emerald-500/10 text-emerald-500' : 'bg-blue-500/10 text-blue-500'}`}>
                          {item.is_outgoing ? '→' : '←'}
                        </div>
                        {idx !== activity24h.length - 1 && (
                          <div className="w-px h-full bg-gray-100 dark:bg-white/5 mt-1" />
                        )}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-bold text-gray-950 dark:text-white truncate">
                            {item.is_outgoing ? 'Sent to' : 'Received from'} {item.sender_name}
                          </span>
                          <span className="text-[10px] text-gray-400 flex-shrink-0">{formatTimeAgo(item.created_at)}</span>
                        </div>
                        <p className="text-[11px] text-gray-400 mt-0.5 truncate">in {item.conversation_title} ({item.account_name})</p>
                        <p className="text-gray-600 dark:text-gray-300 mt-1 line-clamp-2 italic bg-gray-50 dark:bg-white/5 p-2 rounded-lg border border-gray-100 dark:border-white/5">
